...
- Slide 2.4: "A caccia di parametri - Esempio: Ore di Studio e Voto"
- Slide 2.5: "A caccia di parametri - Esempio: La Resistenza"
- Slide 2.6: "A caccia di parametri - Il Paradosso di Monty Hall"
  - Simulatore Monty Hall interattivo
- Slide 2.7: "A caccia di parametri - COVID-19 e decisioni data-driven"
  - Notebook COVID-19 Modeling (opzionale)
- Slide 2.8: "Cos'è l'Intelligenza Artificiale"
- Slide 2.9: "Tipi di apprendimento automatico"
- Slide 2.10: "Il processo di training"
- Slide 2.11: "AI nella didattica STEM"
- Artefatti: Notebook labirinto, Simulatore Monty Hall, Notebook COVID-19, Infografica AI/ML, Glossario
...
##### Slide 2.4: "A caccia di parametri - Esempio: Ore di Studio e Voto"
**Contenuto visivo**: Studenti con libri aperti, orologio, pagella

**Il problema**:
- Vogliamo **predire il voto** di uno studente basandoci sulle **ore di studio**
- Abbiamo raccolto dati di 8 studenti
- Domanda: Se uno studente studia 4.5 ore, che voto prenderà?

**Approccio Machine Learning**:

**Passo 1: Raccogliere i dati**
- Chiediamo a 8 studenti quante ore hanno studiato per una verifica
- Registriamo il voto ottenuto

| Studente | Ore di studio | Voto (su 10) |
|----------|---------------|--------------|
| 1 | 0 | 3 |
| 2 | 1 | 4 |
| 3 | 2 | 5 |
...
- Ipotizziamo una relazione lineare: **Voto = m × Ore + q**
- **Due parametri** da trovare:
  - **m** (pendenza): quanto migliora il voto per ogni ora di studio?
  - **q** (intercetta): voto di partenza senza studiare (preparazione di base)

**Passo 3: Training (Regressione Lineare)**
- Troviamo la retta che meglio si adatta ai dati raccolti
- Minimizziamo l'errore tra predizioni e voti reali
- Risultato: **m ≈ 0.93** e **q ≈ 3.18**
- **Formula trovata**: Voto = 0.93 × Ore + 3.18

**Passo 4: Predizione**
- Ora possiamo predire il voto per 4.5 ore di studio:
- **Voto = 0.93 × 4.5 + 3.18 ≈ 7.4**

**Interpretazione dei parametri**:
- **m = 0.93**: Ogni ora di studio in più vale circa 1 punto sul voto
- **q = 3.18**: Anche senza studiare (0 ore), uno studente prende ~3 (conoscenze pregresse, fortuna, intuizione)

**Visualizzazione grafica**:
- Grafico Ore vs Voto con i punti degli 8 studenti
- Retta di regressione che attraversa i dati
- Punto di predizione evidenziato (4.5 ore → voto 7.4)
- La retta **NON** passa per l'origine (q ≠ 0)

**Collegamento al ML**:
- ✅ **Dati**: coppie (Ore, Voto) raccolte empiricamente
- ✅ **Modello**: Voto = m × Ore + q (lineare con **2 parametri**)
- ✅ **Training**: regressione lineare trova m e q ottimali
- ✅ **Predizione**: usiamo i parametri trovati per nuovi valori

**Riflessione pedagogica**:
- **Correlazione ≠ Causalità**: Studiare di più è correlato a voti migliori, ma non è l'unico fattore!
  - Qualità dello studio (non solo quantità)
  - Metodo di studio
  - Riposo e concentrazione
  - Comprensione vs memorizzazione
- **Limiti del modello**:
  - Non si può studiare 100 ore e prendere 100! Il voto ha un massimo (10)
  - Il modello lineare è una **semplificazione** della realtà
  - Oltre una certa soglia, studiare di più non migliora linearmente
- **Discussione in aula**:
  - Cosa significa q = 3.18? È giusto che anche senza studiare si prenda 3?
  - Cosa succede se uno studente studia 10 ore? Il modello predice ~12.5 (impossibile!)
  - Modelli più realistici: curva logistica (con saturazione)

**Messaggio chiave**: Il ML trova i parametri ottimali del modello attraverso i dati. Ma ogni modello ha limiti e va interpretato criticamente!
...
- Nota: **q = 0** (nessun voltaggio → nessuna corrente)
...
- La retta **passa per l'origine** (0, 0)
...
**Confronto con esempio precedente**:
- **Slide 2.4 (Ore/Voto)**: 2 parametri (m e q ≠ 0) → retta non passa per origine
- **Slide 2.5 (Resistenza)**: 1 parametro (solo m, q = 0) → retta passa per origine
- Quando la fisica/logica ci dice che "zero input → zero output", allora q = 0
...
**Messaggio chiave**: Il ML trova i parametri ottimali del modello attraverso i dati, poi li usa per fare predizioni. A volte abbiamo 1 parametro, a volte 2 (o milioni!).
...
##### Slide 2.7: "A caccia di parametri - COVID-19 e decisioni data-driven"
**Contenuto visivo**: Grafico crescita esponenziale casi/decessi Italia (febbraio-marzo 2020)

**Il contesto storico**:
- **Febbraio 2020**: Primi casi di COVID-19 in Italia
- Crescita rapida e preoccupante dei contagi e decessi
- **Domanda critica**: È necessario un lockdown nazionale? Quando intervenire?
- I decisori politici devono basarsi sui dati per scelte difficili

**Il problema (predittivo)**:
- Abbiamo dati delle **prime 2-3 settimane**
- Dobbiamo **predire** cosa succederà se non interveniamo
- Le conseguenze delle decisioni: vite umane, economia, libertà personali

**Dati reali (Italia, febbraio-marzo 2020)**:

| Data | Decessi totali | Giorni dall'inizio |
|------|----------------|-------------------|
| 21 feb | 2 | 0 |
| 25 feb | 12 | 4 |
| 1 mar | 34 | 9 |
| 5 mar | 148 | 13 |
| 9 mar | 463 | 17 |
| **10 mar** | **631** | **Lockdown nazionale** |

**Approccio Machine Learning (modellazione epidemiologica)**:

**Passo 1: Visualizzare i dati**
- Grafico decessi vs tempo
- Osservazione: non è lineare, sembra esponenziale!

**Passo 2: Scegliere il modello**
- Modello epidemiologico semplificato: **N(t) = N₀ × e^(rt)**
- N(t) = numero decessi al tempo t
- N₀ = valore iniziale
- r = **tasso di crescita** (parametro da trovare)
- t = giorni

**Passo 3: Training (regressione esponenziale)**
- Usiamo i dati delle prime 2 settimane
- Troviamo il parametro r che minimizza l'errore
- Risultato: **r ≈ 0.23-0.25** (raddoppio ogni ~3 giorni!)

**Passo 4: Predizione "senza intervento"**
- Cosa succede se continuiamo così?
- Predizione a 14 giorni: scenario catastrofico
- I modelli mostrano curva insostenibile per il sistema sanitario

**La decisione basata sui dati**:
- **9 marzo 2020**: Governo italiano annuncia lockdown nazionale
- Una delle prime decisioni così drastiche in Europa
- Basata su: dati + modelli predittivi + consulenza scientifica
- Trade-off difficile: salute pubblica vs economia vs libertà

**Cosa è successo dopo il lockdown**:
- La curva si è appiattita (r diminuito)
- Crescita esponenziale rallentata
- Il modello ha guidato anche la decisione di "quando riaprire"

**Collegamento al ML**:
- ✅ **Dati**: Contagi e decessi giornalieri
- ✅ **Modello**: Crescita esponenziale N(t) = N₀ × e^(rt)
- ✅ **Training**: Trovare r dai dati iniziali
- ✅ **Predizione**: Scenari futuri per guidare decisioni
- ✅ **Validazione**: Confronto predizioni vs realtà

**Riflessione etica e sociale**:
- **Potere predittivo**: Il ML può salvare vite umane
- **Responsabilità**: Le predizioni influenzano decisioni critiche
- **Qualità dei dati**: Dati incompleti o errati → predizioni sbagliate
- **Comunicazione**: Come spiegare modelli complessi al pubblico?
- **Incertezza**: I modelli hanno margini di errore, non sono certezze assolute
- **Trasparenza**: Importanza di condividere dati e modelli con la comunità scientifica

**Limiti del modello semplificato**:
- Modello reale molto più complesso (modelli SIR, SEIR, agenti)
- Considera: età popolazione, mobilità, misure preventive, capacità ospedali
- ML moderno: modelli più sofisticati, ma principio base uguale

**Messaggio chiave**: Il Machine Learning non è solo teoria - può guidare decisioni che salvano vite. Con grandi dati vengono grandi responsabilità.

**Discussione in aula** (suggerita):
- Come vi siete sentiti durante quel periodo?
- Pensate che la decisione del lockdown sia stata giusta?
- Quali altri dati avreste voluto avere per decidere meglio?
- Fiducia nei modelli scientifici vs paura/incertezza

**Artefatto collegato**:
- 📓 **Notebook COVID-19 Modeling** (opzionale, avanzato)
- Dati reali Italia (open data Protezione Civile)
- Implementazione regressione esponenziale
- Visualizzazione curve con/senza interventi
- Confronto predizioni vs realtà storica
...
- **Notebook COVID-19 Modeling** (opzionale, avanzato):
  - Dati reali Italia (open data Protezione Civile)
  - Regressione esponenziale su dati storici
  - Visualizzazione curve con/senza interventi
  - Confronto predizioni vs realtà storica
  - Riflessione su etica e responsabilità dei modelli predittivi
