#!/usr/bin/env python3
"""Build demo/xor_live.html with dx compiler modules embedded for Pyodide."""
...
def main():
    ...
    # Build xor_live.html from embedded template
    html = HTML_TEMPLATE.replace("%%DX_MODULES%%", modules_js)
    out_path = os.path.join(DEMO_DIR, "xor_live.html")
    ...
    # Build xor.html from template
    tpl_path = os.path.join(DEMO_DIR, "xor.template.html")
    xor_out = os.path.join(DEMO_DIR, "xor.html")
    if os.path.exists(tpl_path):
        with open(tpl_path, "r") as f:
            xor_html = f.read()
        xor_html = xor_html.replace("%%DX_MODULES%%", modules_js)
        with open(xor_out, "w") as f:
            f.write(xor_html)
        print(f"Generated {xor_out}")
        size_kb = os.path.getsize(xor_out) / 1024
        print(f"Size: {size_kb:.1f} KB")
